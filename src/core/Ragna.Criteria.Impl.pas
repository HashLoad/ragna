...
type
  ...
  private
    ...
    procedure &Equals(const AValue: TGuid); reintroduce; overload;
...
procedure TDefaultCriteria.Equals(const AValue: TGuid);
const
  PHRASE = ' %s %s';
var
  LKeyParam: string;
...
begin
  ...
  FQuery.SQL.Text := FQuery.SQL.Text + Format(PHRASE, [TOperatorType.EQUALS.ToString, ':' + LKeyParam]);

  LParam := FQuery.ParamByName(LKeyParam);
  LParam.DataType := ftGuid;
  LParam.AsGUID := AValue;
end;
