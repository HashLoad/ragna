...
type
  ...
  public
    ...
    function Remove(const AField: TField; const AValue: Int64): {$IFDEF UNIDAC}TUniQuery{$ELSE}TFDQuery{$ENDIF}; overload;
    function Remove(const AField: TField; const AValue: TGuid): {$IFDEF UNIDAC}TUniQuery{$ELSE}TFDQuery{$ENDIF}; overload;
    function Remove(const AField: TField; const AValue: string; const AGuid: Boolean = False): {$IFDEF UNIDAC}TUniQuery{$ELSE}TFDQuery{$ENDIF}; overload;
    function FindById(const AField: TField; const AValue: Int64): {$IFDEF UNIDAC}TUniQuery{$ELSE}TFDQuery{$ENDIF}; overload;
    function FindById(const AField: TField; const AValue: TGuid): {$IFDEF UNIDAC}TUniQuery{$ELSE}TFDQuery{$ENDIF}; overload;
    function FindById(const AField: TField; const AValue: string; const AGuid: Boolean = False): {$IFDEF UNIDAC}TUniQuery{$ELSE}TFDQuery{$ENDIF}; overload;
    function UpdateById(const AField: TField; const AValue: Int64; const ABody: TJSONObject): {$IFDEF UNIDAC}TUniQuery{$ELSE}TFDQuery{$ENDIF}; overload;
    function UpdateById(const AField: TField; const AValue: TGuid; const ABody: TJSONObject): {$IFDEF UNIDAC}TUniQuery{$ELSE}TFDQuery{$ENDIF}; overload;
    function UpdateById(const AField: TField; const AValue: string; const ABody: TJSONObject; const AGuid: Boolean = False): {$IFDEF UNIDAC}TUniQuery{$ELSE}TFDQuery{$ENDIF}; overload;
    ...
    function &Equals(const AValue: TGuid): {$IFDEF UNIDAC}TUniQuery{$ELSE}TFDQuery{$ENDIF}; overload;
...
function TRagnaHelper.Remove(const AField: TField; const AValue: string;
  const AGuid: Boolean): TFDQuery;
var
  LRagna: IRagna;
begin
  LRagna := TRagna.Create(Self);
  LRagna.Remove(AField, AValue, AGuid);
  Result := Self;
end;

function TRagnaHelper.Remove(const AField: TField;
  const AValue: TGuid): TFDQuery;
var
  LRagna: IRagna;
begin
  LRagna := TRagna.Create(Self);
...
function TRagnaHelper.UpdateById(const AField: TField; const AValue: TGuid;
  const ABody: TJSONObject): TFDQuery;
var
  LRagna: IRagna;
begin
  LRagna := TRagna.Create(Self);
...
function TRagnaHelper.UpdateById(const AField: TField; const AValue: string;
  const ABody: TJSONObject; const AGuid: Boolean): TFDQuery;
var
  LRagna: IRagna;
begin
  LRagna := TRagna.Create(Self);
  LRagna.UpdateById(AField, AValue, ABody, AGuid);
  Result := Self;
end;
...
function TRagnaHelper.Equals(const AValue: TGuid): TFDQuery;
var
  LRagna: TRagna;
begin
  LRagna := TRagna.Create(Self);
...
function TRagnaHelper.FindById(const AField: TField;
  const AValue: TGuid): TFDQuery;
var
  LRagna: IRagna;
begin
...
function TRagnaHelper.FindById(const AField: TField; const AValue: string;
  const AGuid: Boolean): TFDQuery;
var
  LRagna: IRagna;
begin
  LRagna := TRagna.Create(Self);
  LRagna.FindById(AField, AValue, AGuid);
  Result := Self;
end;
